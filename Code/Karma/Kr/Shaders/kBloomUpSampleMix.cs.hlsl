...
void Main(uint3 DTid : SV_DispatchThreadID)
{
	...
	if (Pos.x < (uint) Width && Pos.y < (uint) Height)
	{
		...
		float3 HDRColor = TexImage[Pos];
		float3 BlurColor = kBlurUpSampleTent3x3(BlurImage, Sampler, TexCoord, FilterRadius);
		Output[Pos] = lerp(HDRColor, BlurColor, MixStrength);
	}
}
